...
import static com.cybage.ecommerce.Status.*;
...
public class AmazonOnlineService extends ECommerceServiceGrpc.ECommerceServiceImplBase {
    ...
    public void placeOrder(OrderRequest request, StreamObserver<OrderResponse> responseObserver) {
       // categoryHandler.get(request.getOrderCategory()).accept(responseObserver);
        if(request.getOrderCategory().equals(Category.ELECTRONICS)){
            System.err.println("Order category :"+request.getOrderCategory());
            this.handleElectronics(responseObserver);
        }else if(request.getOrderCategory().equals(Category.EBOOKS)){
            System.err.println("Order category :"+request.getOrderCategory());
            this.handleEBooks(responseObserver);


        }else{
            System.err.println("Order category is mismatch :"+request.getOrderCategory());
        }
